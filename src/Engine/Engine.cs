        ...
        internal int Start()
        {
            ...
            void E_CheckGame()
            {
                ...
                if (remainingTime.TotalSeconds <= 10 && !bWarningSound)
                {
                    ...
                    if (warningSound != null)
                        warningSound.Play("./Files/GameAboutToEnd.wav");
                }
                ...
                {
                    if (foundStatues[i])
                        continue; // `continue` loop if ith statue is already found

                    for (int j = 0; j < mazeFinish[i].Count; j++)
                    {
                        bool bStatueProcd =
                            (int)fPlayerX == mazeFinish[i][j].x
                            && (int)fPlayerY == mazeFinish[i][j].y;
                        if (bStatueProcd)
                        {
                            foundStatues[i] = true;
            ...
            void E_RenderFrame()
            {
                ...
                for (int x = 0; x < nScreenWidth; x++)
                {
                    ...
                    while (!bHitWall && fDistanceToWall < fDepth)
                    {
                        ...
                        int nCeiling = (int)(
                            (float)(nScreenHeight / 2.0f) - nScreenHeight / ((float)fDistanceToWall)
                        );
                ...
                for (int i = 0; i < statues.Count; i++)
                {
                    ...
                    float fDistance = Vector2.Distance(
                        new(fPlayerX, fPlayerY),
                        new(statues[i].x, statues[i].y)
                    );

                    int nCeiling = (int)(
                        (float)(nScreenHeight / 2.0f) - nScreenHeight / ((float)fDistance)
                    );
                    int nFloor = nScreenHeight - nCeiling;

                    string[] statueSprite = [];
                    if (foundStatues[i]) // statue is already found
                    {
                        switch (fDistance)
                        {
                            case float n when n <= 02f: statueSprite = statueFoundSprite1; break;
                            case float n when n <= 04f: statueSprite = statueFoundSprite2; break;
                            case float n when n <= 06f: statueSprite = statueFoundSprite3; break;
                            case float n when n <= 08f: statueSprite = statueFoundSprite4; break;
                            default: statueSprite = []; break;
                        }
                    }
                    else // statue is not yet found
                        switch (fDistance)
                        {
                            case float n when n <= 02f: statueSprite = statueSprite1; break;
                            case float n when n <= 04f: statueSprite = statueSprite2; break;
                            case float n when n <= 06f: statueSprite = statueSprite3; break;
                            case float n when n <= 10f: statueSprite = statueSprite4; break;
                            default: statueSprite = []; break;
                        }

                    float diff =
                        fAngle < fFOVAngle && fFOVAngle - 2f * MathF.PI + fFOV > fAngle
                            ? fAngle + 2f * MathF.PI - fFOVAngle
                            : fAngle - fFOVAngle;
                    ...
                    for (int y = 0; y < statueSprite.Length; y++)
                    {
                        for (int x = 0; x < statueSprite[y].Length; x++)
                        {
                            if (statueSprite[y][x] is not '`')
                            {
                                ...
                                if (
                                    0 <= screenX
                                    && screenX <= nScreenWidth - 1
                                    && 0 <= screenY
                                    && screenY <= nScreenHeight - 1
                ...
                for (int y = 0; y < screen.GetLength(1); y++)
                {
                    ...
                    for (int x = 0; x < screen.GetLength(0); x++)
                    {
                        ...
                        if (x == 0)
                            render.Append(' ', center); // center screen
            ...
            void E_HandleConsoleSize()
            {
                while (true)
                {
                    ...
                    // handle invalid console size
                    bConsoleLargeEnough =
                        nConsoleWidth >= nScreenWidth && nConsoleHeight >= nScreenHeight;
