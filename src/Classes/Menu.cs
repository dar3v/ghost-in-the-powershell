            ...
            while (true)
            {
                ...
                if (currentWidth >= titlePrompt[0].Length && currentHeight >= titlePrompt.Length)
                {
                    int verticalOffset = Math.Max(
                        0,
                        (currentHeight / 2) - (titlePrompt.Length / 2)
                    );
                    Console.Clear();
                    ...
                    for (int i = 0; i < titlePrompt.Length; i++)
                    {
                        if (Console.KeyAvailable && Console.ReadKey(true).Key == ConsoleKey.Enter)
                        {
                            ...
                            {
                                int horizontalOffsets = Math.Max(
                                    0,
                                    (currentWidth - lines.Length) / 2
                                );
                        ...
                        foreach (char c in line)
                        {
                            ...
                            )
                            {
                                ...
                                {
                                    int remainingHorizontalOffset = Math.Max(
                                        0,
                                        (currentWidth - remainingLine.Length) / 2
                                    );
                                    Console.WriteLine(
                                        new string(' ', remainingHorizontalOffset) + remainingLine
                                    );
                                }
                                return;
                            }
